...
- **News auto-ingest:** see **[DEPLOY_CHECKLIST.md](DEPLOY_CHECKLIST.md)** for SQL blocks, Edge Function secrets, `news-sync` deploy, curl test, and GitHub cron secrets.
- Schema block in `supabase/supabase-schema.sql` (`news.ingest_source`, `news_feed_sources`, `news_ingest_log`). Edge Function secrets: `SERVICE_ROLE_KEY`, `NEWS_SYNC_SECRET`.
- Scheduled sync: `.github/workflows/news-sync.yml` (every 6h) needs repo secrets `NEWS_SYNC_URL` and `NEWS_SYNC_SECRET`.
